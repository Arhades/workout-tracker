...
export async function DataView(ctx) {
  ...
  async function doExport() {
    ...
    const a = el('a', { href: url, download: `workout-backup-${db.todayISO()}.json` })
    a.click()
  ...
  async function doImport(file) {
    try {
      ...
      if (!confirm('Import this backup? Sessions/check-ins with matching dates will be replaced.')) return
  ...
  async function refresh() {
    const [sessions, sets, readiness, techniques, program] = await Promise.all([
